...
struct Ortho_Projection_Matrix {
    ...
    Ortho_Projection_Matrix(float left, float right, float bottom, float top, float near, float far)
    {
        auto linear_index = [](i32 r, i32 c) -> std::size_t {
            return r*4 + c;
        };
        ...
        mat[linear_index(2, 0)] = -2.0f / (far - near);
        mat[linear_index(2, 1)] = 0.0f;
        mat[linear_index(2, 2)] = 0.0f;
        mat[linear_index(2, 3)] = -(far + near) / (far - near);
