...
uniform sampler2D u_gpos_texture;
uniform sampler2D u_gdiffuse_texture;
uniform sampler2D u_gnormal_texture;
...
vec3 calc_dir_light(Shared_Data shared_data)
{
    ...
    float shadow = 1.0f; //calc_shadow_value(normalize(u_directional_light.pos - shared_data.frag_pos), 0);
...
{
    vec3 light_direction = normalize(spot_light.pos-shared_data.frag_pos);
    ...
    float shadow = 1.0f; // calc_shadow_value(normalize(spot_light.pos - shared_data.frag_pos), shadow_index);
    return (ambient + shadow*(diffuse + specular)) * shared_data.sampled_diffuse;
}

vec3 calc_point_light(Point_Light pl, Shared_Data shared_data)
{
    vec3 light_direction = normalize(pl.pos - shared_data.frag_pos);
    float dis = distance(pl.pos, shared_data.frag_pos);

    float attn = u_point_light_intensity / (dis*dis);
...
void main()
{
    ...
    shared_data.norm = texture(u_gnormal_texture, texture_coordinates).rgb;
    shared_data.sampled_diffuse = texture(u_gdiffuse_texture, texture_coordinates).rgb;
    shared_data.frag_pos = texture(u_gpos_texture, texture_coordinates).rgb;
    shared_data.view_dir = normalize(u_camera_pos - shared_data.frag_pos);
