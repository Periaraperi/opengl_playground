...
in VS_DATA {
    vec3 frag_pos; // in world space, not used anywhere
    vec3 tanget_space_frag_pos;
    vec3 tanget_space_point_light_pos;
    vec3 tanget_space_camera_pos;
    vec3 normal; // not really used for this demo
...
struct Point_Light {
    vec3 pos; // NOT USED HERE, pos is set in vertex shader
...
{
    vec3 light_direction = normalize(vs_data.tanget_space_point_light_pos - vs_data.tanget_space_frag_pos);
    float dis = distance(vs_data.tanget_space_point_light_pos, vs_data.tanget_space_frag_pos);
...
void main()
{
    shared_data.view_dir = normalize(vs_data.tanget_space_camera_pos - vs_data.tanget_space_frag_pos);
